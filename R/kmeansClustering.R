...
# angepasst an Mdbt und Doku standards
  ...
  #    requireNamespace('cclust')
  	...
  	Cls=ClusterRename(Cls,Data)
      return(list(Cls=Cls,Object=list(SumDistsToCentroids=SSE,Centroids=res$centers)))
    },
    "Steinley"={
    ...
    message('Currently, the "Type" parameter of k-means cannot be set in the case of using a distance matrix.')
    return(kmeansDist(Distance=DataOrDistance, ClusterNo=ClusterNo,Centers=Centers,RandomNo=RandomNo, PlotIt=PlotIt,verbose = Verbose,...))
  }
}
